...
class API2:
    def __init__(self):
        pass
    id = ''
    def get_all(self, phone):
        count_field = 0
        dictObj = {}
        user = API1.user(phone).get_all()
        # print(user)
        self.id = user['infor']['id']
        level1 = ''
        level2 = ''
        level3 = ''
        level4 = ''
...
if __name__ == "__main__":
    ...
    for i in list_phone:
        ...
        print(run_time_user)
    print("avg time / person: " + str(total_time/API1.user.count))


    do_phu.append(is_age / API1.user.count * 100)
    do_phu.append(is_gender / API1.user.count * 100)
    do_phu.append(is_relationship / API1.user.count * 100)
    do_phu.append(is_child / API1.user.count * 100)
    do_phu.append(is_job / API1.user.count * 100)
    do_phu.append(is_edu / API1.user.count * 100)
    do_phu.append(is_language / API1.user.count * 100)
    do_phu.append(is_hometown / API1.user.count * 100)
    do_phu.append(is_location / API1.user.count * 100)
    do_phu.append(is_own / API1.user.count * 100)
    do_phu.append(is_nganhang / API1.user.count * 100)
    do_phu.append(is_baohiem / API1.user.count * 100)
    do_phu.append(is_vay / API1.user.count * 100)
    do_phu.append(is_the / API1.user.count * 100)
    do_phu.append(is_dautu / API1.user.count * 100)
    do_phu.append(is_suckhoe / API1.user.count * 100)
    do_phu.append(is_giaoduc / API1.user.count * 100)
    do_phu.append(is_dulichnuocngoai / API1.user.count * 100)
    do_phu.append(is_dulichtrongnuoc / API1.user.count * 100)
    do_phu.append(is_duhoc / API1.user.count * 100)
    do_phu.append(is_thethao / API1.user.count * 100)
    do_phu.append(is_anuong / API1.user.count * 100)
    do_phu.append(is_khoahoc / API1.user.count * 100)
    do_phu.append(is_nghethuat / API1.user.count * 100)
    do_phu.append(is_game / API1.user.count * 100)
    do_phu.append(is_sothichsang / API1.user.count * 100)
    do_phu.append(is_xeco / API1.user.count * 100)
    do_phu.append(is_thoitrang / API1.user.count * 100)
    do_phu.append(is_nhac / API1.user.count * 100)
    do_phu.append(is_thucpham / API1.user.count * 100)
    do_phu.append(is_phim / API1.user.count * 100)
    do_phu.append(is_lamdep / API1.user.count * 100)
    df = DataFrame({'field': field, 'độ phủ(%)': do_phu})
    df.to_excel(r'check do phu post.xlsx', encoding='utf-8')
    df_get.to_excel(r'get 1000 truong post only.xlsx', encoding='utf-8')
